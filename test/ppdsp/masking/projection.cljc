...
(defn make-random-projection-with-noise-and-translation-mask-factory-cycles 
  ...
  (def counter (atom 0))
  (def logistic-counter (atom (- (* -1 cycle-size) 1 )) )
  (fn [schema-features]
    (let [seed (or seed 1)
          rng (seeded-rng seed)
          translation (or translation 0)
          ...
          mask-factory (decorate-mask-factory
                        ...
                        (fn [features]
                           (when features (swap! logistic-counter inc)   
                                  ...
                                  (if (> @logistic-counter (* 1 cycle-size))
                            ...
                            (let [
                                 ...
                                 ;; logistic-sigma  (/ maximum-fn-value  (+ 1    (m/exp (* (* -1 growth-rate-k)  @logistic-counter ) )  )  )
                                  cumulative-noise   (if (= maximum-fn-value 0)
                                                     ;; (repeatedly projection-features #(m/abs (next-gauss! rng 0 cumulative-sigma)))  ;;absolute values are taking to both adddition and subtraction waruni
                                  ...
                                  (if (> @counter (* 2 cycle-size))
                                       (do
                                           (reset! counter 1 )  
								                                   ...
								                                   ;;Reset noise --> low privacy and high accuracy
								                                   ;;without resetting --> High noise/privacy and low accuracy
                                             (reset! cumulative-noise-vector-atom (m/zero-vector (count schema-features))) 
                                    )
                                 )
																	                                    ;; (cond            ;;;Addition and subtraction of noise
